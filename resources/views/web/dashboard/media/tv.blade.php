<div class="relative space-y-10 mb-10" wire:init="load">
    ...
    <div class="relative -m-5 h-64">
        ...
        <div class="absolute inset-0 flex items-center m-5">
            <div class="flex items-end space-x-5">
                ...
                <div class="mb-5">
                    <h1 class="text-xl font-medium line-clamp-2">{{$media['name'] ?? 'Unknown Title' }}</h1>
                    <span class="text-sm text-gray-400 font-medium">{{$media['number_of_seasons']}} {{ $str::plural('Season', $media['number_of_seasons']) }} - {{$media['number_of_episodes']}} {{ $str::plural('Episode', $media['number_of_episodes']) }}</span>
                </div>
            </div>
        </div>
    </div>

    @livewire('web.dashboard.media.components.controls', ['media' => $media, 'mediaType' => 'tv'])
    ...
    <x-section.default title="Seasons">
        <div>
            <x-grid.media.seasons :media="$media['seasons']" mediaType="tv" wire:loading.remove />
